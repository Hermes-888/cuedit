/*
    ...
    TODO:
        might be able to use vtt instead of panels?
        add file managment: New, Open, Save
        positioning? align, vertical, ...
        Context menu?
        Ext.js version?
        Icon font?
    ...
    http://subtitle-horse.com/preview
...
var currentTime = 0.0;
var adjustment = 0.1;
var filename = 'samplecues.vtt';
var track = document.getElementById("cctrack").track;// .track?
var vidplayer = document.getElementById('myVid');
...
vidplayer.addEventListener("loadedmetadata", function() {
   //track = this.addTextTrack("captions", "English", "en");
    console.log('loadedmetedata');

    // track cues
    if (track) {
        // Set the track to be active
        //track.track.mode='showing';//'hidden';// showing displays on video like captions
        console.log('track:', track, track.cues);
        // create panels from track cues
        for(var i=0; i<track.track.cues.length; i++) {
            var cue = track.track.cues[i];
            if (track.track.cues[i].id == '') {
                track.track.cues[i].id = i;// needed for cuechange
            }
        ...
        track = this.addTextTrack("captions", "English", "en");
        track.mode = "showing";
        track.addCue(new VTTCue(2, 8.1, "[Test]"));// track.track???
        //addCuePanel();
        console.log('dynamic track:', track);
    }
    track.addEventListener("cuechange", seeActiveCue);
    function seeActiveCue(e) {
        console.log('act e:', this.activeCues.length, e);
        //if (this.track.activeCues.length > 0) {
        if (this.activeCues.length > 0) {
            //console.log('cuechange:', this.track.activeCues[0]);
            currentPanel = this.activeCues[0].id;
...
$(document).on('keypress', function(e) {
    ...
    if (e.which == 45 || e.which == 95) {
        ...
        if (currentButton) {
            ...
            if (track) {
                if (type == 'IN') {
                    track.track.cues[currentPanel].startTime = time;
                }
                if (type == 'OUT') {
                    track.track.cues[currentPanel].endTime = time;
                }
            }
        }
    }
    if (e.which == 43 || e.which == 61) {
        // + or = inc currentime & repeat if mouse still down?
        if (currentButton) {
            time = parseFloat($(currentButton).attr('id'));
            time += adjustTime;
            timeStr = time.toFixed(3);
    ...
    if (e.altKey && e.which == 8) {
        ...
        if (currentPanel) {
            ...
            if (track) {
                track.track.removeCue(track.track.cues[currentPanel]);
            }
    ...
    if (e.altKey && e.which == 105) {
        ...
        if (currentPanel) {
            ...
            if (track) {
                track.track.addCue(new VTTCue(vidplayer.currentTime, vidplayer.currentTime, 'testing text in new cue'));
            }
    ...
    if (e.altKey && e.which == 109) {
        ...
        if (!togglend) {
            ...
            // pause, update end cue time and focus on text area
            vidplayer.pause();
            togglend = false;
            ...
            if (track) {
                track.track.cues[currentPanel].endTime = currentTime;
            }
            //focus on text?
...
function addCuePanel(cue) {
    ...
    // In time btn id="IN_"+cue.endTime
    ...
    // click the button to jump to the marker
    ...
    $(txt).on('keyup', function(e) {
        if (track) {
            track.track.cues[currentPanel].text = $(this).val();
        }
    ...
    $(dbtn).on('click', function(e) {
        ...
        var current = parseInt(darr[1]);
        console.log('deleteAt:',current);
        panels.splice(current, 1);
        if (track) {
            track.track.removeCue(track.track.cues[current]);
        }
        updateMarkers(current);
    });
    ...
    $(ibtn).on('click', function(e) {
        ...
        panels.splice(current, 0, pobj);
        if (track) {
            track.track.addCue(new VTTCue(currentTime, currentTime+0.5, 'testing text in new cue'));
        }
        updateMarkers(current);
    });
}
...
function updateMarkers(current) {
    ...
    if (track) {
        console.log('track.cues:', track.track.cues.length, track.track.cues);
    }
}

$('#savefile').on('click', writefile);
function writefile() {
    ...
    if (panels.length>0) {
        ...
        for(var i=0; i<panels.length; i++){
            vttdata += panels[i].panel+'\n';
            vttdata += formatTime(panels[i].startTime)+' --> '+formatTime(panels[i].endTime)+'\n';
            vttdata += panels[i].text+'\n\n';//end of cue
        }

        // .time Must be adjusted after : it doesn't matter if cue is at the end of the file
        //vttdata += 'NOTE\nA simple note\n\n';

        console.log('Write file:', filename, vttdata);
        //$.post('js/writefile.php', {filename:filename,vttdata:vttdata}, writedone);
    }
}
